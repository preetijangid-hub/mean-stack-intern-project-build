...
export class Dashboard implements OnInit {
  ...
  tasks: Task[] = []; loading = false; creating = false; updating = ''; deleting = ''; error = ''; success = ''; query = ''; filter: 'all' | 'open' | 'completed' = 'all'; showForm = false; editingTask: Task | null = null; title = ''; description = '';
  ngOnInit(): void { this.loadTasks(); }
  get user() { return this.authService.getUser(); }
  get visibleTasks(): Task[] { const q = this.query.trim().toLowerCase(); return this.tasks.filter((task) => `${task.title} ${task.description || ''}`.toLowerCase().includes(q) && (this.filter === 'all' || (this.filter === 'completed' ? task.completed : !task.completed))); }
  ...
  loadTasks(): void { this.loading = true; this.error = ''; this.taskService.getTasks().pipe(finalize(() => { this.loading = false; this.changeDetector.detectChanges(); })).subscribe({ next: (response) => this.tasks = response.tasks, error: (error) => this.handleError(error, 'Unable to load tasks. Please try again.') }); }
