...
export class Login {
  ...
  submit(form: NgForm): void { if (form.invalid) { form.control.markAllAsTouched(); return; } this.loading = true; this.error = ''; this.auth.login(this.email.trim(), this.password).pipe(finalize(() => { this.loading = false; })).subscribe({ next: () => this.router.navigateByUrl('/dashboard'), error: (error) => { this.error = error.status === 0 ? 'The live API could not be reached.' : error.error?.message || 'Login failed. Please check your credentials.'; this.loading = false; this.changeDetector.detectChanges(); } }); }
}