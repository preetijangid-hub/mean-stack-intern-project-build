...
export class Dashboard implements OnInit {
  ...
  loadTasks(): void {
    ...
    this.taskService
      ...
      .subscribe({
        ...
        error: (error) =>
          this.handleError(error, 'Unable to load tasks. Please try again.'),
      });
  }
  openCreateForm(): void {
  ...
  saveTask(form: NgForm): void {
    if (form.invalid || this.creating || this.updating) {
    ...
    if (this.editingTask) {
      this.updateTask(this.editingTask, data);
      return;
    }
    ...
    this.taskService
      ...
      .subscribe({
        ...
        error: (error) =>
          this.handleError(error, 'Unable to create task. Please try again.'),
      });
  }

  updateTask(task: Task, data: { title: string; description: string }): void {
    if (!task._id) return;

    this.updating = task._id;
    ...
    this.taskService
      ...
      .subscribe({
        ...
        error: (error) =>
          this.handleError(error, 'Unable to update task. Please try again.'),
      });
  }

  toggleTask(task: Task): void {
    if (!task._id || this.updating || this.deleting) return;
    ...
    this.taskService
      ...
      .subscribe({
        next: (response) => {
          ...
          this.success = response.task.completed
  ...
  deleteTask(task: Task): void {
    if (!task._id || !confirm(`Delete "${task.title}"?`)) return;

    this.deleting = task._id;
    ...
    this.taskService
      ...
      .subscribe({
        ...
        error: (error) =>
          this.handleError(error, 'Unable to delete task. Please try again.'),
      });
  }
